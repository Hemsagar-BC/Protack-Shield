(function(){
  ...
  var el = document.getElementById('ripple-grid-bg');
  if(!el) return;
  ...
  var vsrc = 'attribute vec2 p;varying vec2 v;void main(){v=p*0.5+0.5;gl_Position=vec4(p,0,1);}';
  var fsrc = [
  ...
  function cs(s,ty){var sh=gl.createShader(ty);gl.shaderSource(sh,s);gl.compileShader(sh);
    if(!gl.getShaderParameter(sh,gl.COMPILE_STATUS)){console.error(gl.getShaderInfoLog(sh));return null;}return sh;}
  var vsh=cs(vsrc,gl.VERTEX_SHADER), fsh=cs(fsrc,gl.FRAGMENT_SHADER);
  if(!vsh||!fsh)return;
  ...
  var pa=gl.getAttribLocation(pg,'p');
  ...
  var u={};['t','r','ri','gs','gt','fd','vs2','gi','op','mp','mi','mr','gc'].forEach(function(n){u[n]=gl.getUniformLocation(pg,n);});

  gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);

  var mx=0.5,my=0.5,tx=0.5,ty2=0.5,mInf=0,curInf=0,W=1,H=1;
  ...
  document.addEventListener('mousemove',function(e){
    ...
    ty2=1.0-e.clientY/window.innerHeight;
    mInf=1.0;
  });

  function draw(tm){
    var time=tm*0.001;
    mx+=(tx-mx)*0.1;my+=(ty2-my)*0.1;
    ...
    gl.uniform3f(u.gc, 0.2, 0.05, 1.0);
