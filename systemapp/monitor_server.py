...
DEFAULT_DEVICES = {
    ...
    "urban": {
        ...
        "cctv-cam-01": {"type": "cctv_camera", "status": "online", "health": 100},
        "cctv-cam-02": {"type": "cctv_camera", "status": "online", "health": 100},
        "nvr-recorder-01": {"type": "nvr_recorder", "status": "online", "health": 100},
    }
}
...
def cctv_endpoint():
    """Urban CCTV surveillance endpoint"""
    data = request.json or {}
    attack_type = data.get("attack_type", "feed_tampering")
    device_id = data.get("device_id") or get_target_device()
    ip = request.remote_addr

    # Higher damage for CCTV attacks (surveillance is critical)
    damage = 20 if attack_type == "camera_disable" else 15
    dev_status = update_device_health(device_id, damage=damage)

    is_blocked = random.random() < SECTOR_BLOCK_RATE
    action = "BLOCKED" if is_blocked else "DETECTED"

    target_msg = f"Target: {device_id}" if device_id else "Target: General"
    print(f"{'🚫' if is_blocked else '⚠️'} [CCTV] {attack_type} attack from {ip}. {target_msg}. Status: {action}")

    send_security_event("cctv_attack", {
        "domain": "urban",
        ...
        "ip": ip,
        "action": action,
        "blocked_by": "NETWORK_SHIELD" if is_blocked else None,
        "threat_type": "cctv_" + attack_type
    }, attacker_ip=ip)

    if is_blocked:
        return jsonify({"error": f"CCTV {attack_type} blocked", "blocked": True, "device_id": device_id}), 403
    return jsonify({"status": "received", "warning": f"cctv_{attack_type}_detected", "device_id": device_id, "health": dev_status["health"] if dev_status else None}), 200
...
def receive_attack():
    ...
    elif attack_type == "cctv":
        cctv_attack_type = payload.get("attack_type", "feed_tampering")
        send_security_event("cctv_attack", {
            "domain": "urban",
            "attack_type": cctv_attack_type,
            "ip": attacker_ip,
            "device_id": device_id,
            "device_health": dev_status["health"] if dev_status else None,
            "routed": True
